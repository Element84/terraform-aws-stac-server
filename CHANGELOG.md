...
- For local builds or stac-server versions without a release asset (prior to v5.0.0), set `stac_server_zip_filepath` to a local lambda dist ZIP to bypass the download.
...
- `deploy_local_stac_server_artifacts` and the `zip_filepath` field on `api_lambda` / `ingest_lambda` / `pre_hook_lambda` are gone. Remove any references; use `stac_server_zip_filepath` for a local ZIP instead.

### Added


- `stac_server_zip_filepath` variable for pointing at a local stac-server lambda dist ZIP instead of downloading a release asset
...
- `zip_filepath` field from the `api_lambda`, `ingest_lambda`, and `pre_hook_lambda` variables; use `stac_server_zip_filepath` instead
