...
- `stac_server_zip_filepath` variable for pointing at a local stac-server lambda dist ZIP
  instead of downloading a release asset
- `utils/fetch-lambda-dist.bash`, which downloads the lambda dist ZIP from a stac-server GitHub
  release (requires `curl` and `jq` at apply time)
- `utils/build-historical-ingest.bash`, which builds the historical-ingest module's `lambda.zip`
  (previously part of `utils/update-lambdas.bash`)

### Changed

- stac-server lambda ZIPs are no longer built and committed to this repository. The single lambda
  dist ZIP attached to [stac-server releases](https://github.com/stac-utils/stac-server/releases)
  (v5.0.0 and later) is downloaded at apply time and used by the api, ingest, and pre-hook lambdas.
  For older or custom stac-server versions, provide a ZIP via `stac_server_zip_filepath`
- `stac_server_version` is now required and selects the release to download
- Default lambda handlers changed to the dist ZIP entrypoints: `api/index.handler`,
  `ingest/index.handler`, and `pre-hook/index.handler`
- Existing deployments will see an in-place code and configuration update of the api, ingest, and
  pre-hook lambdas on the first apply after upgrading (new ZIP filename, source hash, and handler)
...
- `zip_filepath` field from the `api_lambda`, `ingest_lambda`, and `pre_hook_lambda` variables;
  use `stac_server_zip_filepath` instead
- Committed lambda ZIPs (`lambda/api`, `lambda/ingest`, `lambda/pre-hook`) and
